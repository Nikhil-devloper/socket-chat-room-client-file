...
function App() {
  ...
  const onRoomSelect = (roomName) => {
    ...
    setMyRoom(roomName);
    //you can call callback 
    socket.emit("join_room", {roomName: roomName,username: username});
    //receiverRef.current = username;    
  ...
  useEffect(() => {
    ...
    socket.on("new_message", (data) => {         
      setRoomMessage(prevRoomMessages => ([...prevRoomMessages,data]))
    });
  ...
  const createGroupRoom = (groupName) => {
  ...
  return (
    <div className="App">
      <header className="app-header">
        <img src={logo} alt="" />
        <div className="app-name b-500 primaryColor">My Room Chat</div>
      </header>
