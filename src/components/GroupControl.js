  ...
  return (
    <div>
      <div className="online-users-header">
        ...
        <div style={{ margin: "0 10px" }}>{roomName+ ' Room'}</div>
      ...
      <div className="message-area">
        <ul>
          ...
          {roomMessages && roomMessages.map((msg,index) => {
              return(
                <li style={{
                  flexDirection: username === msg.sender ? "row-reverse" : "row",
                }}
                key={index}
              >
                {msg.type == 'new-user' && <div className='center'> {msg.msg} </div> }

                {msg.type == 'plain-message' &&
                <>
